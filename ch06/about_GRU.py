# GRU:LSTMと同様に勾配消失問題に対する耐性がある。さらに、LSTMよりも内部構造が簡単なので計算量が少ない。
# ゲートは更新ゲートとリセットゲートの2つ

# GRUとLSTMの、どちらが良いかというのは学習データによる。
# 汎化に必要なデータ量がそれほど必要ない。学習が早い->GRU
# 充分なデータ量がある状態。-> LSTM


# https://www.nltk.org/ からデータをダウンロードする必要がある。
