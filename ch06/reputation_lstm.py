...
#学習を始める前に、データの探索的分析をおこなう。具体的にはコーパスに幾つの一意な単語があるか、各文にいくつの単語が含まれているかを確認する。
DATA_DIR = "./data"
LOG_DIR = "./logs"
...
#一意な単語数をlen((word_freqs))から語彙数を決定し、ほかのすべての単語を語彙外の単語として扱う。
#これにより、学習時には現れなかった単語を、予測時に語彙外の単語として扱うことができる
print(maxlen)            # 42
print(len(word_freqs))   # 2313

# 1 is UNK, 0 is PAD
# We take MAX_FEATURES-1 features to account for PAD
# RNNへの入力の各行は単語インデックスの系列であり、インデックスは学習データセット内の頻度によって順序付けられる。
# 2つのルックアップテーブルは単語が与えられた場合に対応するインデックスをルックアップする。
...
# 系列長をMAX_SENTENCE_LENGTHに合わせる。
# 今回は2値なのでラベルは処理する必要がない。
